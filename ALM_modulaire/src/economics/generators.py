...
def generer_rendements_avec_backetest(mu_e, sigma_e, mu_b, sigma_b, corr, dates, date_pivot, nb_sims, asset_equity=None, asset_bond=None, rng=None):
    """
    ...
    if idx_split > 0:
        if asset_equity is not None and asset_bond is not None:
            ...
            if r_eq_h is not None and len(r_eq_h) > 0:
                ...
                # Fallback si chargement CSV échoue : tirage BS sur toute la zone backtest
            ...
            # Fallback si pas d'asset : tirage BS commun à toutes les simulations
            chocs_histo = rng.multivariate_normal([0, 0], cov, size=idx_split)
            r_eq_h = (mu_e*dt - 0.5*s_e**2) + chocs_histo[:, 0]
            r_bd_h = (mu_b*dt - 0.5*s_b**2) + chocs_histo[:, 1]
            r_eq_past = np.tile(r_eq_h.reshape(-1, 1), (1, nb_sims))
    ...
    # Partie Forecast (divergente) — tire du RNG, donc reproductible au niveau
    # global (via GLOBAL_SEED) et divergente entre sims au sein d'un run.
    nb_mois_futur = nb_total_mois - idx_split
    if nb_mois_futur > 0:
        chocs_futur = rng.multivariate_normal([0, 0], cov, size=(nb_mois_futur, nb_sims))
