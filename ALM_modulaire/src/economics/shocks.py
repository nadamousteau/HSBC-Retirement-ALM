...
def ajouter_chocs_merton(rendements_eq, rendements_bd, rendements_inf, nb_periodes, nb_sims):
    """
    ...
    for t in range(nb_periodes):
        ...
        choc_inflation = crise * np.random.normal(settings.SEVERITE_INFLATION_MOYENNE, settings.SEVERITE_INFLATION_SIGMA, nb_sims)

        rendements_eq[t] += choc_eq
        rendements_bd[t] += choc_bd
        rendements_inf[t] += choc_inflation
    
    return rendements_eq, rendements_bd, rendements_inf

def injecter_crise_localisee(r_eq, r_bd, r_inf, dates_list, date_depart, params):
    """
    ...
    if abs((dates_pd[idx] - pd.Timestamp(date_depart)).days) > 40:
        return r_eq, r_bd, r_inf
    
    # Choc initial
    # Actions et obligations : Chute des marchés (log-return pour cohérence)
    r_eq_m[idx, :] = np.log(1.0 - params.get('drop_eq', 0.3))
    r_bd_m[idx, :] = np.log(1.0 - params.get('drop_bd', 0.0))
    
    # Inflation : Pic soudain (choc additif). 
    r_inf_m[idx, :] += params.get('spike_inf')
    ...
    r_inf_m[idx+1:end, :] *= params.get('facteur_vol_inf', 1.0)
    
    return r_eq_m, r_bd_m, r_inf_m