...
from src.analytics import plotting, metrics_decumulation, metrics, metrics_end, scoring
...
def main():
    ...
    # Dictionnaire pour centraliser les KPIs ALM de chaque stratégie pour le MCDA
    kpis_alm_globaux = {}
    ...
    for strat_actuelle in strategies_run:
        ...
        # -------------------------------------------------------------------------
        # 4.1 DÉCUMULATION STOCHASTIQUE ET MODÉLISATION DU PASSIF
        # -------------------------------------------------------------------------
        nom_profil_dec = getattr(settings, 'PROFIL_DECUMULATION_ACTIF', 'EQUILIBRE')
        profil_config = settings.PROFILS_DECUMULATION[nom_profil_dec]

        # Exécution du moteur de décumulation stochastique
        # Note technique temporaire : TAUX_LIVRET_A et TAUX_INFLATION sont passés en scalaires.
        # À remplacer par une matrice stochastique pour un vrai test de Guyton-Klinger.
        mat_cap_retraite, mat_rentes, durees_vie = liabilities.decumulation.simuler_decumulation(
            capitaux_finaux, 
            dernier_salaire, 
            settings.TAUX_LIVRET_A, 
            settings.TAUX_INFLATION, 
            profil_config, 
            horizon_max=settings.DUREE_RETRAITE
        )

        # Reconstitution de la matrice des taux de remplacement pour la compatibilité des graphiques
        taux_remp = mat_rentes / dernier_salaire

        # Calcul vectoriel du Capital Cible (Besoin en Passif)
        taux_remp_cible = profil_config["taux_remplacement_cible"]
        taux_retrait_init = profil_config["taux_retrait_initial"]
        
        rente_cible_vecteur = dernier_salaire * taux_remp_cible
        capital_cible_vecteur = rente_cible_vecteur / taux_retrait_init

        # Calcul des KPIs ALM stochastiques
        kpis_alm = analytics.metrics_decumulation.executer_analyse_alm(
            capitaux_finaux, 
            mat_cap_retraite, 
            dernier_salaire, 
            capital_cible_vecteur, 
            gamma=profil_config["gamma_aversion"]
        )
        
        # Ajout de l'indicateur de transmission (Capital résiduel médian)
        capital_transmission_median = np.median(mat_cap_retraite[-1, :])

        # Stockage standardisé pour le moteur de scoring
        kpis_alm_globaux[strat_actuelle] = {
            "prob_ruine": kpis_alm['probabilite_ruine'],
            "expected_shortfall": kpis_alm['expected_shortfall_5pct'],
            "equivalent_certain": kpis_alm['equivalent_certain'],
            "transmission": capital_transmission_median
        }

        # -------------------------------------------------------------------------
        # 4.2 ANALYSE DE FIN DE CARRIÈRE
        # -------------------------------------------------------------------------
        kpis_fin_acc = analytics.metrics_end.calculer_kpis_fin_accumulation(mat_cap, dates)
        synth_fin_acc = analytics.metrics_end.synthetiser_kpis_fin_accumulation(kpis_fin_acc)
        ...
        # =========================================================================
        # 4.5. REPORTING CONSOLE (INDIVIDUEL PAR STRATÉGIE)
        # =========================================================================
        if getattr(settings, 'PRINT_PERFORMANCE_GLOBALE', False) or getattr(settings, 'PRINT_METRIQUES_RISQUE', False):
            ...
            if getattr(settings, 'PRINT_METRIQUES_RISQUE', False):
                ...
                print(f"  • VaR 95% (Capital P5)      : {kpis['var_95']:>15,.0f} €")
        ...
        if getattr(settings, 'PRINT_KPI_DECUMULATION', False):
            print("\n" + "-"*30)
            print("ANALYSE DE DÉCUMULATION (KPI ALM)")
            print("-"*30)
            print(f"  • Ruine Prob.      : {kpis_alm['probabilite_ruine']*100:.2f} %")
            print(f"  • Expected Sh.     : {kpis_alm['expected_shortfall_5pct']:,.0f} €")
            print(f"  • CEW (Utilité)    : {kpis_alm['equivalent_certain']:,.0f} €")
            
            tr = kpis_alm['taux_remplacement']
            print(f"  • Taux Remp. (P5)  : {tr['p5_pessimiste']*100:.2f} %")
            print(f"  • Taux Remp. (P50) : {tr['p50_median']*100:.2f} %")
            print(f"  • Taux Remp. (P95) : {tr['p95_optimiste']*100:.2f} %")
            print("-"*30 + "\n")

        if getattr(settings, 'PRINT_KPI_END_CARRIERE', False):
            print("\n[ ANALYSE FIN DE CARRIÈRE (5 DERNIÈRES ANNÉES) ]")
            print(f"  • Max Capital sur 5 ans          : {synth_fin_acc['mediane_max_5ans']:>15,.0f} €")
            print(f"  • Bénéfice vs pire point (5 ans) : {synth_fin_acc['mediane_benefice']:>15,.0f} €")
            print(f"  • Manque à gagner vs ATH (5 ans) : {synth_fin_acc['mediane_manque']:>15,.0f} €")


    # =========================================================================
    # ÉVALUATION MCDA : RECHERCHE DE LA MEILLEURE STRATÉGIE
    # =========================================================================
    if getattr(settings, 'TROUVER_LA_MEILLEURE_STRAT', False) and len(strategies_run) > 1:
        nom_profil_dec = getattr(settings, 'PROFIL_DECUMULATION_ACTIF', 'EQUILIBRE')
        profil_config = settings.PROFILS_DECUMULATION[nom_profil_dec]
        
        scores_finaux, meilleure_strat = scoring.calculer_scores_strategies(
            kpis_alm_globaux, profil_config
        )
        
        print("\n" + "★"*80)
        print(f"ANALYSE MULTICRITÈRE (MCDA) - PROFIL ACTIF : {nom_profil_dec}")
        print("★"*80)
        for strat, score in scores_finaux.items():
            print(f"  • Score global {strat:<20} : {score*100:>6.2f} / 100")
        
        print("-" * 80)
        print(f"  ▶ STRATÉGIE OPTIMALE RECOMMANDÉE : {meilleure_strat}")
        print("★"*80 + "\n")
